...
class DemoActor(Actor, DemoActorInterface, Remindable):
    ...
    4. Actor timer
    """
    def _init_(self, ctx, actor_id):
        super(DemoActor, self)._init_(ctx, actor_id)

    async def _on_activate(self) -> None:
        """An callback which will be called whenever actor is activated."""
        print(f'Activate {self._class.name_} actor!', flush=True)

    async def _on_deactivate(self) -> None:
        """An callback which will be called whenever actor is deactivated."""
        print(f'Deactivate {self._class.name_} actor!', flush=True)
