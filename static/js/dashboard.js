...
function showBalance() {
  web3.eth
    .getBalance(address, (err, balance) => {
    ...
    .then(function (events) {
      if (events.length == 0)
        ...
        for (var j = 0; j < events.length; j++) {
          var ptype;
          if (events[j].returnValues.permission == "0") ptype = "Read";
          else ptype = "Modify";
          ...
          contract.methods
            ...
            .then(function (req_address) {
              ...
              contract
                ...
                .then(function (docs) {
                  if (!docs.length) {
                    // check the length, if 0, not shared
                    $("#main-loader").show();
                    contract.methods
                      .getUseraccessKey()
                      .call()
                      .then(function (mkeyHash) {
...
function sendShareMailAjax(doc_id, email, doc_name) {
  ...
  contract.methods
    .getAddressByEmail(email)
    .call()
    .then(function (requester_address) {
      contract.methods
        .getEmailIdByUsrAddr(address)
        .call()
        .then(function (owner_email_name) {
          contract.methods
            .getPublicKey(requester_address)
            .call()
            .then(function (req_pub_key) {
              contract.methods
                .getDocIndex(doc_id, address)
                .call()
                .then(function (docIndex) {
                  var data = {
...
$(document).on("click", ".verify_doc", function () {
  ...
  contract
    ...
    .then(function (vlist) {
      if (vlist.length == 0) {
        contract.methods
          .getDocumentName(doc_id, doc_owner)
          .call()
          .then(function (docname) {
            ...
            $("#verifyThisDoc").submit(function (e) {
              ...
              $("#main-loader").show();
              contract.methods
                .verifyUserDocument(doc_id, doc_owner)
                .send()
                .then(function () {
                  swal({
                    title: "Success!",
            ...
            for (var j = 0; j < docGroup[property].length; j++) {
              ...
              $(
                ...
                `<tr>
                        ...
                        <td>${ptype}</td>
                        <td><a class="btn verify_doc" doc_id = "${
                          docGroup[property][j].docId
                        }" docOwner = "${property}">
